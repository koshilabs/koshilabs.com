        ...
        <>
            ...
            >
                <div className="flex items-center gap-6 mb-8">
                    {service.icon}
                    <h3 className="text-3xl font-semibold text-white bg-clip-text">{service.title}</h3>
                </div>
                <p className="text-gray-300 flex-grow text-lg leading-relaxed">{service.description}</p>
                <Button 
            ...
            <Modal
                ...
                title={service.title}
                icon={React.cloneElement(service.icon as React.ReactElement, { className: "w-8 h-8" })}
            >
                <div className="text-gray-300" dangerouslySetInnerHTML={{ __html: service.detailedDescription }} />
            </Modal>
        </>
    );
};
